...
revenue_ranks as ( select film_id, -- This CTE is to rank all the films by total revenue ( amont + tax) in descending order, by category
    title, 
    category,
    total_film_revenue,
    dense_rank() over (
partition by category order by total_film_revenue desc) as revenue_rank -- Imortant! You generally cannot referenc an alias within a window function. 
from revenue_cte                                                        -- But this gets a pass since this alias (total_film_revenue) is from another CTE.
)
